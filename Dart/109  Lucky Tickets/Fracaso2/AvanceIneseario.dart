...
MapEntry<String, int> SumDigits(String number, int cantOfdigits) {
...
Map<int, List<String>> generateCounterSet(Map<String, int> SumsOfNumbers) {
  Map<int, List<String>> CountOfSums = {};
  SumsOfNumbers.forEach((key, element) {
    if (!CountOfSums.containsKey(element)) {
      CountOfSums[element] = [key];
    } else {
...
void main() {
  ...
  List<int> Numbers = List.generate(generateRange, (index) => index + 1);
  print(Numbers);

   Map<String,int> SumsOfNumbers = {};
  
  Numbers.forEach((element) {
      MapEntry<String, int> entry = SumDigits(DecimaltoBase(element, base), digits);
      SumsOfNumbers.addEntries([entry]);

   });

   SumsOfNumbers.forEach((key, value) { 
    print("$key : $value");
  });
