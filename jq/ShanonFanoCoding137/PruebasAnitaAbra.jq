...
def checkIndex(indexXelementXsplit):
  ...
  | indexXelementXsplit[2] as $split
  | if $split < $element then  
    $index - 1
    else 
    $index
    end   
...
def splitArrays(ArraysSumsXsplitValXindexElemtXStartXend):
  ...
  | .[] as $Index
  | checkIndex([$Index, $ArraysSums[$Index], $splitVal]) as $splitIndex  
...
def codingLetter(LettersbyappearanceXindexElement):
  ...
  (arraySumation([0,$size,$newLettersbyAppearance,$newStart,$newEnd]) | (.[-1] / 2) | round)]
  ,
  #Start 2
  .[1][0] as $newArraySums 
  | .[2] as $newStart
