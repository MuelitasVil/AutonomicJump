...
def ValidBlanckspaces(blankspaces; voidspaces; cantOfwords):
  ...
  | if $mod <= 1  then
      if (((voidspaces / blankspaces) | floor) == (cantOfwords - 1)) then 
        true
      elif ((((voidspaces / blankspaces) | floor) + 1) == (cantOfwords - 1)) then 
        true
      elif (((((voidspaces - 1) / blankspaces) | floor) + 1) == (cantOfwords - 1)) then 
        true
      else 
        false
      end  
...
def verifyWords(cantOfletters;totalLetters;cantOfwords):
    ...
    | 2 as $blankspaces
    | if ($voidspaces  == ($cantOfwords - 1)) or (($voidspaces  == $cantOfwords)) then 
      true
    else  
      [$blankspaces]
      | until (
...
def insertBlankSpaces(line;totalLetters):
  ...
  | if ($voidspaces - $cantOfletters) == ($cantOfwords - 1) then
    1
    else 
    [2]
    | until (
      (ValidBlanckspaces(.[0]; $voidspaces; $cantOfwords) or (.[0] > 1)); 
      [
      .[0] as $newIndex
      | ($newIndex + 1)
      ])
