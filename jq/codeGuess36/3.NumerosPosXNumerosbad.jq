...
#En este punto verico que si estoy asociando correctamente los numeros de la posicion
# Con los numeros que no van en esta

def discardBandNumbers(information):
    ...
    | if ([($badNumbers)| index(.[1])] | length) >= 1  then [.[1], $badNumbers] else "Nop" end; 
  ...
  | if (($pares | length) >  1) then $pares # eliminar el string vacio al final de _
    ...
    | if (($nums | length) == 1) then [(["000", $nums] | add), $tried[1]] # Compruebo que tamaño tiene cada numero 
      ...
      | if $tried[1] == "1" then ([1,$number[$num], $num ]) # Si existe un numero correcto en el intento lo agrupo con un 1 al inicio
       ...
       | $badNumbers 
       | discardBandNumbers([$attemptByPosition, $badNumbers]);
