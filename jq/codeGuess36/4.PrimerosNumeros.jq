...
def uniqueNumber(possibleNumbers):
    DeletePosition(possibleNumbers)  # Ahora solo tengo el numero y si este puede ser una respuesta
    | add as $PosibleAndNumber
    | $PosibleAndNumber # Uno toda esta informacion a un solo arreglo
    | indices(1) as $CantNumbers
    | if ($CantNumbers | length) == 1 then $PosibleAndNumber[$CantNumbers[0]+1] | tostring else null end; # Verifico cuantos numeros puede ir en esa posicion si solo es uno retornelo
 ...
 | .[] | split("_")] 
  ...
  | if (($pares | length) >  1) then $pares # eliminar el string vacio al final de _
    ...
    | if (($nums | length) == 1) then [(["000", $nums] | add), $tried[1]] # Compruebo que tamaño tiene cada numero 
      ...
      | $badNumbers 
      | discardBadNumbers([$attemptByPosition, $badNumbers]) as $possibleNumbers
      | uniqueNumber($possibleNumbers) as $Rta
      | $Rta;
